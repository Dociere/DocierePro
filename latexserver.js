...
function runPdfLatex(texFilePath, outputPath) {
    return new Promise((resolve, reject) => {
        ...
        pdflatex.on('error', (error) => {
            ...
            reject(new Error(`Failed to start pdflatex: ${error.message}. Make sure pdflatex is installed and in PATH.`));
        });
    });
}
...
app.post('/api/latex/compile', async (req, res) => {
    ...
    try {
        const { latex, isTemp = true, fileName = 'temp', format = 'pdf' } = req.body;
...
\\end{document}`;
        ...
        if (format === 'image' || format === 'png') {
            try {
                ...
                console.error('⚠️ Image conversion failed, falling back to PDF:', imageError.message);
...
app.get('/api/equations/list', async (req, res) => {
    ...
    try {
        ...
        equations.sort((a, b) => new Date(b.lastModified) - new Date(a.lastModified));
...
app.delete('/api/equations/:filename', async (req, res) => {
    ...
    try {
        ...
        if (error.code === 'ENOENT') {
            console.log('❌ Equation file not found for deletion:', req.params.filename);
            return res.status(404).json({ error: 'Equation file not found' });
        }
