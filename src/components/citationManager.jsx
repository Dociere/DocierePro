...
import { TbX, TbCopy, TbDeviceFloppy, TbListNumbers, TbSearch, TbLoader } from "react-icons/tb";
...
const CitationManager = ({ onClose }) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const autoFileName = formData.title
        ? formData.title.replace(/[^a-zA-Z0-9 ]/g, "").replace(/\s+/g, "_").substring(0, 50)
        : `citation_${Date.now()}`;
    ...
    try {
      ...
      if (data.success) {
        if (isDoi) {
          ...
          setSearchStatus({ type: "success", message: `Found: ${data.data.title}` });
        } else {
        ...
        setSearchStatus({ type: "error", message: data.error || "Search failed" });
      }
    } catch (err) {
  ...
  const selectResult = useCallback((result) => {
    ...
    setSearchResults([]);
    setSearchQuery("");
    setSearchStatus({ type: "success", message: `Selected: "${result.title}"` });
  }, []);
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex bg-gray-100 p-1 rounded-lg">
              <button
                ...
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === "create" ? "bg-white text-black shadow-sm" : "text-gray-500 hover:text-gray-900"
                  }`}
              >
                Create Reference
              </button>
          ...
          {activeTab === "create" ? (
            <>
                ...
                <div className="space-y-6 max-w-xl mx-auto">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-2 relative">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {isSearching ? <TbLoader size={16} className="animate-spin" /> : "Search"}
                      </button>
                    </div>
                    ...
                    {searchResults.length > 0 && (
                      <div className="absolute left-0 right-0 top-full mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl z-[110] max-h-80 overflow-y-auto overflow-x-hidden p-2 space-y-1">
                        <div className="flex justify-between items-center px-3 py-1 mb-1 border-b border-gray-100">
                          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Select a Paper</span>
                          <button onClick={() => setSearchResults([])} className="text-gray-400 hover:text-black transition-colors"><TbX size={14} /></button>
                        </div>
                        {searchResults.map((res, idx) => (
                          ...
                          >
                            <h5 title={res.title} className="text-sm font-semibold text-gray-900 line-clamp-1 group-hover:text-black mb-0.5">{res.title}</h5>
                            <div title={`${res.authors} • ${res.journal} • ${res.year}`} className="flex items-center gap-2 text-[11px] text-gray-400 font-medium">
                              <span className="truncate max-w-[150px]">{res.authors}</span>
                    ...
                    {searchStatus && (
                      <p className={`mt-2 text-xs font-medium ${searchStatus.type === "success" ? "text-green-600" : "text-red-500"
                        }`}>
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="grid grid-cols-3 gap-3 col-span-2">
                      <div>
                        <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1.5">Vol</label>
                        <input name="volume" value={formData.volume} onChange={handleInputChange} className="w-full bg-white px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gray-400 focus:outline-none" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1.5">Issue</label>
                        <input name="issue" value={formData.issue} onChange={handleInputChange} className="w-full bg-white px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gray-400 focus:outline-none" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1.5">Pages</label>
                        <input name="pages" value={formData.pages} onChange={handleInputChange} className="w-full bg-white px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gray-400 focus:outline-none" />
                      </div>
                    </div>

                    <div className="col-span-1">
                      <label className="block text-[11px] font-bold text-gray-500 uppercase mb-2">Year</label>
                      <input name="year" value={formData.year} onChange={handleInputChange} className="w-full bg-white px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gray-400 focus:outline-none" />
                    </div>
                    <div className="col-span-1">
                      <label className="block text-[11px] font-bold text-gray-500 uppercase mb-2">DOI</label>
                      <input name="doi" value={formData.doi} onChange={handleInputChange} className="w-full bg-white px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gray-400 focus:outline-none" />
                    </div>
                  </div>
              ...
              <div className="flex-1 flex flex-col p-8 bg-gray-50">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-bold text-gray-700 uppercase tracking-widest">Reference Preview</h3>
                  <div className="flex gap-2">
                    <button
                      onClick={() => copyToClipboard(latexCode, "LaTeX copied!")}
                      disabled={!latexCode}
                ...
                <div className="flex-1 bg-white border border-gray-200 rounded-xl shadow-sm relative overflow-hidden flex items-center justify-center">
                  {previewUrl ? (
                    ...
                    <div className="text-center p-12">
                      ...
                      <p className="text-sm font-semibold text-gray-500 uppercase tracking-widest mb-1">Preview Area</p>
                      <p className="text-xs text-gray-400 max-w-[200px] leading-relaxed">Fill the form and generate to see your reference here.</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="w-full flex flex-col overflow-hidden p-8 bg-gray-50">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 mb-1">Citation Library</h3>
                  <p className="text-sm text-gray-500">Manage and export your saved references</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {savedCitations.map((citation, index) => (
                    ...
                    >
                          ...
                          <div className="flex items-start gap-3 mb-2">
                            ...
                            <div>
                              ...
                              <p className="text-[13px] text-gray-500 font-medium">
                                {citation.authors} • <span className="text-black font-semibold">{citation.year}</span>
                              </p>
                            </div>
                          </div>

                          <div className="flex items-center gap-3 text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-11">
                            <span className="bg-gray-50 px-2 py-0.5 rounded border border-gray-100">{citation.format}</span>
                            <span className="truncate max-w-[300px]">{citation.journal || "No Journal"}</span>
                          </div>
                        </div>
                      ...
                      <div className="mt-5 pt-4 border-t border-gray-100 flex flex-wrap gap-2">
                        <button
                          onClick={() => copyToClipboard(`[${index + 1}]`, "Citation number copied!")}
                        ...
                        <button
                          onClick={() => copyToClipboard(`^{[${index + 1}]}`, "Superscript copied!")}
                        ...
                        <button
                          onClick={() => copyToClipboard(citation.latexCode, "Full reference copied!")}
                          className="px-3 py-1.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-all text-[11px] font-bold inline-flex items-center gap-1.5 shadow-sm"
                        >
                  ...
                  {savedCitations.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-2xl border border-dashed border-gray-300">
                      ...
                      <h4 className="font-bold text-gray-800 text-sm">Library is empty</h4>
                      <p className="text-xs text-gray-400 mt-1">Saved references will appear here.</p>
                    </div>
                  )}
                </div>
              </div>
