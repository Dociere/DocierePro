...
import "react-quill-new/dist/quill.snow.css";
import "../utils/latexBlots.jsx";
import { getRichTextHandlers } from "./richTextToolbar.jsx";
import SlateEditorPanel from "./slateEditor.jsx"; // 🚀 Slate Integration
...
import { isMainFile } from "../utils/latexUtility.jsx";
...
// Global Event Triggers for the Insert Modals
const fireInsertEvent = (eventName) => {
  document.dispatchEvent(
    new CustomEvent(eventName, { detail: { quill: null } }),
  );
};

// --- SLEEK KEBAB MENU COMPONENT ---
const ActionMenu = ({
  onMoveUp,
  onMoveDown,
  onDelete,
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-1 w-48 bg-white border border-slate-200 rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.08)] z-50 py-1.5 font-inter text-sm text-slate-700">
          {/* {!hideStructureActions && ( */}
          {/* <> */}
          <button
            onClick={() => {
...
const RecursiveSection = ({
  ...
  // 🚀 Mode can be: "code", "visual" (Slate), or "legacy" (Quill)
  ...
  // Handler for Legacy Quill
  const handleRichTextChange = (html) => {
    const rawLatex = richTextToSection(html);
    handleContentChange(rawLatex);
  };

  // 🚀 Handler for Modern Slate
  const currentAst = useMemo(
    () => ({
      type: "root",
      content: textToContentNodes(localLatex) || [],
    }),
    [localLatex],
  );
  ...
  const handleSlateChange = useCallback(
    (newAst) => {
      const newLatex = printAstToLatex(newAst);
      handleContentChange(newLatex);
    },
    [handleContentChange],
  );
  ...
  const handleTitleChange = (e) => {
    ...
    if (section.macroNode) {
      const clonedMacro = structuredClone(section.macroNode);
      setSectionTitle(clonedMacro, newTitle);
      onUpdate({ ...section, name: newTitle, macroNode: clonedMacro });
    }
  };

  const isLeadingContent = section.type === "content";
  const isEnvironment = section.type === "environment";
  const childType =
  ...
  const canHaveChildren = !isEnvironment && childType !== "paragraph";
  const siblingType = isEnvironment ? "section" : section.type;
  ...
  const shouldHideStructure = !isMainFile && level === 0;
  ...
  return (
    ...
    >
      {/* HEADER BAR */}
      {!isLeadingContent && (
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 bg-slate-50/80 rounded-t-xl group">
          <div className="flex items-center gap-2 flex-1">
            <button
              onClick={() => setIsExpanded(!isExpanded)}
              className="p-1 text-slate-400 hover:text-slate-800 hover:bg-slate-200 rounded-md transition-all duration-200 flex-shrink-0"
              style={{
            ...
            {isEnvironment ? (
              ...
              <input
                ...
                className={`bg-transparent border-none focus:ring-0 outline-none w-full text-slate-800 placeholder-slate-300 py-1 transition-colors hover:text-blue-600 focus:text-blue-600 ${headerClasses}`}
              />
            )}
          </div>

          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 mr-2 opacity-60 hover:opacity-100 transition-opacity">
              <button
                onClick={() => fireInsertEvent("trigger-insert-table")}
                title="Insert Table"
                className="p-1.5 text-slate-500 hover:bg-slate-200 hover:text-blue-600 rounded-md transition-all"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              <button
                onClick={() => fireInsertEvent("trigger-insert-image")}
                title="Insert Image"
                className="p-1.5 text-slate-500 hover:bg-slate-200 hover:text-emerald-600 rounded-md transition-all"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              <button
                onClick={() => fireInsertEvent("trigger-insert-math")}
                title="Insert Math"
                className="p-1.5 text-slate-500 hover:bg-slate-200 hover:text-purple-600 rounded-md transition-all font-serif italic text-sm font-bold"
              >
                ∑
              </button>
            </div>
            ...
            {/* 🚀 3-WAY TOGGLE PILL */}
            <div className="flex items-center bg-slate-200/80 p-0.5 rounded-lg border border-slate-200 shadow-inner">
              <button
            ...
            <ActionMenu
              ...
              onAddSibling={() => onAddAfter(siblingType)}
              sectionType={siblingType}
              isFirst={isFirst}
              isLast={isLast}
              hideStructureActions={shouldHideStructure}
            />
          </div>
        </div>
      )}

      {/* LEADING CONTENT EXCEPTION */}
      {isLeadingContent && (
        <div className="flex items-center justify-between mb-3 px-1">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setMode("code")}
              className={`text-[10px] font-bold uppercase tracking-wider border px-3 py-1 rounded-md shadow-sm transition-all ${
                mode === "code"
      ...
      {/* CONTENT AREA */}
      {isExpanded && (
        <div className={`flex flex-col gap-4 ${isLeadingContent ? "" : "p-5"}`}>
          <div className="min-h-[150px] border border-slate-200 rounded-lg overflow-hidden focus-within:border-slate-400 focus-within:ring-4 focus-within:ring-slate-50 transition-all bg-white shadow-sm">
            {/* 🚀 Rendering Logic for the 3 modes */}
            {mode === "code" ? (
              <div className="h-[250px] w-full">
                <MonacoEditorPanel
                  value={localLatex}
                  handleLatexChange={handleContentChange}
                  monacoEditorRef={{ current: null }}
                  readOnly={false}
                />
              </div>
            ) : (
              <div className="h-[400px] w-full bg-white relative slate-section-wrapper flex flex-col">
                <SlateEditorPanel
                  globalAst={currentAst}
          ...
          {/* CHILDREN SECTIONS */}
          {section.children && section.children.length > 0 && (
            <div className="mt-4 ml-2 pl-6 border-l-2 border-slate-100 flex flex-col gap-4">
              {section.children.map((child, i) => (
                <RecursiveSection
                  key={child.id}
                  section={child}
                  ...
                  onDuplicate={() => {
                    ...
                    clone.id = `ast-sec-${Date.now()}`;
                    newChildren.splice(i + 1, 0, clone);
                    onUpdate({ ...section, children: newChildren });
                  }}
                  onAddAfter={(specificType) => {
                    const newChildren = [...section.children];
                    const newMacro = parseLatexToAst(
                      `\\${specificType}{New ${specificType}}`,
                    ).content[0];
                    newChildren.splice(i + 1, 0, {
                      id: `ast-sec-${Date.now()}`,
                      type: specificType,
                      name: `New ${specificType}`,
                      macroNode: newMacro,
                      contentNodes: [],
                      children: [],
                    });
          ...
          {canHaveChildren && (
            <button
              ...
              className="mt-1 self-start flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-semibold text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-md transition-all"
            >
              <svg
                className="w-4 h-4"
                fill="none"
...
const SectionEditor = ({
  ...
  if (!sections || sections.length === 0) {
    return (
      <div className="flex h-full items-center justify-center text-slate-400 font-inter">
        <div className="flex flex-col items-center gap-4">
          ...
          <button
            onClick={() => {
              const newMacro = parseLatexToAst(`\\section{New section}`)
                .content[0];
              onSectionsChange([
                {
                  id: `ast-sec-${Date.now()}`,
            ...
            className="px-4 py-2 bg-slate-800 text-white rounded-lg shadow hover:bg-slate-700 transition-colors"
          >
  ...
  const handleUpdate = (index, updatedSection) => {
    const newSections = [...sections];
  ...
  const handleMove = (index, direction) => {
    ...
    if (target >= 0 && target < newSections.length) {
  ...
  const handleDuplicate = (index) => {
    const newSections = [...sections];
    const clone = structuredClone(sections[index]);
    clone.id = `ast-sec-${Date.now()}`;
    newSections.splice(index + 1, 0, clone);
    onSectionsChange(newSections);
  };

  const handleAddAfter = (index, type) => {
    const newSections = [...sections];
    const newMacro = parseLatexToAst(`\\${type}{New ${type}}`).content[0];
  ...
  return (
    <div className="h-full w-full bg-slate-50 overflow-y-auto pt-8 pb-40">
      <style>{`
        .ql-toolbar.ql-snow { border: none; border-bottom: 1px solid #e2e8f0; background: #f8fafc; font-family: 'Inter', sans-serif; border-radius: 8px 8px 0 0; }
        .ql-container.ql-snow { border: none; font-family: 'Inter', sans-serif; font-size: 14px; }
        .ql-editor { min-height: 150px; color: #334155; line-height: 1.6; padding: 20px; }
        .ql-editor h2 { font-weight: 600; color: #0f172a; margin-top: 1em; }
        .ql-editor h3 { font-weight: 600; color: #1e293b; margin-top: 1em; }

        /* 🚀 Compact overrides specifically for Slate inside the Section Blocks */
        .slate-section-wrapper .overflow-y-auto { padding: 1rem 1.5rem !important; }
        .slate-section-wrapper > div { border-radius: 0 0 0.5rem 0.5rem; }
        .slate-section-wrapper .sticky.top-0 { border-radius: 0.5rem 0.5rem 0 0; border-bottom: 1px solid #e2e8f0; padding: 0.5rem; }
      `}</style>

      <div className="max-w-4xl mx-auto px-6 font-inter">
        {sections.map((section, index) => (
          <RecursiveSection
            key={section.id}
            section={section}
            ...
            onAddAfter={(type) => handleAddAfter(index, type)}
            sectionToRichText={sectionToRichText}
            richTextToSection={richTextToSection}
          />
        ))}
        ...
        {isMainFile && (
          <button
            onClick={() => handleAddAfter(sections.length - 1, "section")}
            className="w-full mt-6 py-4 border-2 border-dashed border-slate-300 rounded-xl text-slate-400 hover:text-slate-600 hover:border-slate-400 hover:bg-slate-100 transition-all font-semibold flex items-center justify-center gap-2 tracking-wide"
          >
            <svg
              className="w-5 h-5"
              fill="none"
