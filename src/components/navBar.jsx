...
const NavBar = () => {
  ...
  return (
    <>
      ...
      >
        <div className={`h-11 w-full top-[3px] bottom-0 border-b-[0.5px] flex transition-colors duration-300 ${isDark
          ? 'bg-[#252525] border-[#404040]'
          : 'bg-[#F9F9F9] border-[#CFCFCF]'
          }`}>
          {/* Left Part - Title bar Menus */}
          <div
            className={`flex flex-1 gap-7 text-sm pl-5 transition-colors ${isDark ? 'text-[#e5e5e5]' : 'text-[#212121]'
              }`}
          ...
          {projectDetails?.currentProject?.title ? (
            <div
              ...
              className={`flex flex-1 py-0 text-sm font-inter font-medium flex-row justify-center items-center cursor-pointer transition-colors ${isDark ? 'text-[#e5e5e5]' : 'text-black'
                }`}
              style={{ WebkitAppRegion: "no-drag" }}
            >
              {projectDetails?.currentProject?.title}
              <EditIcon
                style={{ fill: isDark ? "#a0a0a0" : "#585858", WebkitAppRegion: "no-drag" }}
                className="ml-2 w-3 h-3 relative select-none"
              />
              {renameProject && (
                <div className={`absolute mt-20 px-2 py-2 border-2 select-none rounded-md flex flex-row z-10 transition-colors ${isDark ? 'bg-[#2d2d2d] border-[#404040]' : 'bg-[#EAEAEA] border-[#CFCFCF]'
                  }`}>
                  <input
                    className={`pl-2 w-96 transition-colors ${isDark ? 'bg-[#1a1a1a] text-[#e5e5e5]' : 'bg-white text-black'
                      }`}
                    type="text"
                    placeholder="Enter"
                    value={updatetitle}
          ...
          >
            <button
              className={`my-2 rounded-md px-3 transition-colors ${isDark ? 'hover:bg-[#404040] text-white' : 'hover:bg-gray-200 text-[#0a0a0a]'
                }`}
            ...
            <button
              className={`my-2 rounded-md px-3 transition-colors ${isDark ? 'hover:bg-[#404040]' : 'hover:bg-gray-200'
                }`}
