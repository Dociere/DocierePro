...
//   const [pdfPageWidth, setPdfPageWidth] = useState(595); // A4 default in PDF points
//   const viewportRef = useRef(null);
//   const pageRefs = useRef([]);
//   const zoomDropdownRef = useRef(null);
//   const pageInputRef = useRef(null);
//   const savedPageRef = useRef(1); // Persist page across recompilations
//   const { settings } = useSettings();

//   // Keep ref in sync with pageNumber state
//   useEffect(() => {
...
//     const containerWidth = viewportRef.current.clientWidth - 32; // 16px padding each side
//     const fitScale = Math.round((containerWidth / pdfPageWidth) * 100) / 100;
//     const clamped = Math.max(0.5, Math.min(4, fitScale));
//     setIsFitMode(true);
//     setZoomDropdownOpen(false);
//     // Use rAF to batch the scale update and avoid layout thrashing
//     requestAnimationFrame(() => setScale(clamped));
//   }, [pdfPageWidth]);

//   // Zoom-to-fit label for the dropdown button
...
//     // Restore page position after recompilation
//     const targetPage = Math.min(savedPageRef.current, loadedNumPages);
//     if (targetPage > 1) {
//       // Delay slightly to let the document render first
//       setTimeout(() => setPageNumber(targetPage), 100);
//     }
//   }

//   // Track intrinsic page width for zoom-to-fit calculation
//   const handlePageLoadSuccess = (page) => {
//     if (page.width && page.width !== pdfPageWidth) {
...
//           // Re-fit on window resize if in fit mode
//           if (isFitMode) {
//             const containerWidth = viewportRef.current.clientWidth - 32;
//             const fitScale =
//               Math.round((containerWidth / pdfPageWidth) * 100) / 100;
//             setScale(Math.max(0.5, Math.min(4, fitScale)));
//           }
//         }
//       });
//     };
...
//   // Ctrl+0 shortcut for Zoom to fit
//   useEffect(() => {
...
//         {/* Zoom dropdown and download - only visible when PDF exists */}
//         {pdfUrl && (
//           <div
//             className="flex items-center gap-2"
//             style={{
